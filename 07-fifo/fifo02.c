...
int main(){
   ...
   switch (fork()){ //creo un hijo

      case -1: //si hay error entonces me genera un error y sale con -1
      ...
      case 0:
         ...
         fifo_d = open(FIFO_PATH, O_RDONLY, 0);  // abro la fifo en modo leer solamente  O_RDONLY
         ...
         // Se lee FIFO
         leido = read(fifo_d, buff, sizeof(buff)); // leo de lo que hay en la fifo y lo pongo en buff 
         if(leido == -1){
            ...
            write(STDOUT_FILENO, buff, leido-1); // muestro en pantalla lo que está en el buffer 
            write(STDOUT_FILENO, "\"\n", sizeof("\"\n"));
         }
         close(fifo_d); // cierro la fifo 
